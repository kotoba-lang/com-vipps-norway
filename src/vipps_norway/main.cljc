(ns vipps-norway.main
  "Kotodama WASM entrypoint for the Vipps_norway clean-room actor (L4) — Clojure port.
  ...
  facts (`vipps_norway.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "vipps_norway")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"vipps_norway.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "vipps_norway-compat" :tier tier :entities entities} 200])
...
(defn start! [] :vipps-norway-compat/ready)
